# Aproveitamento das equipes da série C no mês de Fevereiro de 2024 (considerando só jogos oficiais)
...
times_serieC2024_geral <- read_ods("Central da Serie C 2024/jogos dos times da serie C 2024.ods")

colnames(times_serieC2024_geral)


aproveitamento_times_seriec_fev24 <- times_serieC2024_geral %>%
...
aproveitamento_times_seriec_fev24
...
aproveitamento_times_seriec_fev24$time <- factor(aproveitamento_times_seriec_fev24$time, 
                                                 levels = unique(aproveitamento_times_seriec_fev24$time))
...
aproveitamento_times_seriec_fev24 %>%
  ...
  geom_label(aes(label = percent(ranking_aproveitamento, accuracy = 0.1)),
             ...
             vjust = 0.5, size = 2.5, hjust = 0.3) +
  labs(title = "Aproveitamento times da Série C em Fevereiro 2024",
       x = "Times",
       y = "Aproveitamento")


# Jogos disputados por cada equipe em Fevereiro 2024:

jogos_disputados_fev24 <- times_serieC2024_geral %>%
