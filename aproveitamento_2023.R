# Aproveitamento das equipes da série C no ano de 2023 (considerando só jogos oficiais)
...
aproveitamento_times_seriec_2023 <- times_serieC2023_geral %>%
...
aproveitamento_times_seriec_2023
...
aproveitamento_times_seriec_2023$time <- factor(aproveitamento_times_seriec_2023$time, 
                                                 levels = unique(aproveitamento_times_seriec_2023$time))
...
aproveitamento_times_seriec_2023 %>%
  ...
  labs(title = "Aproveitamento times da Série C em 2023",
       x = "Times",
       y = "Aproveitamento")


# Jogos disputados por cada equipe em 2023:

jogos_disputados_2023 <- times_serieC2023_geral %>%
  select(time, meses, qtd_unidade) %>%
  group_by(time) %>%
  summarise(jogos_disputados = sum(qtd_unidade)) %>%
  arrange(desc(jogos_disputados))

jogos_disputados_2023
